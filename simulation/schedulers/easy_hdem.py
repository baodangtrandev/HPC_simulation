...
class Easy_hdem(BatsimScheduler):
    ...
    def onJobSubmission(self, job):
        ...
        predict_walltime = HDEMWalltimePrediction(self.finishedJobs, job)
    ...
    def convert_to_float(self, value):
        if isinstance(value, float):
            ...
            return [self.convert_to_float(v) for v in value]
        elif isinstance(value, dict):
            return {k: self.convert_to_float(v) for k, v in value.items()}
        elif isinstance(value, np.float32):  # Nếu bạn đang sử dụng NumPy
            return float(value)  # Chuyển đổi về float
        return value

    def onSimulationEnds(self):
        outputStatisticFileName = "prediction-statistic.json"
        try:
            self.predictionStatistics = self.convert_to_float(self.predictionStatistics)
